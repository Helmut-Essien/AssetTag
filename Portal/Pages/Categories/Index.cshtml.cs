...
namespace Portal.Pages.Categories
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            //return Page();
            // Use GetAsync so we can inspect the response status instead of letting GetFromJsonAsync throw on 401
            var response = await _httpClient.GetAsync("api/categories");

            if (response.StatusCode == HttpStatusCode.Unauthorized)
            {
                // Trigger the authentication challenge (middleware can redirect to login)
                return Challenge();
            }

            if (!response.IsSuccessStatusCode)
            {
                // Surface a friendly error and return the page with an empty list
