...
namespace Portal.Pages
{
    ...
    public class IndexModel : PageModel
    {
        ...
        private async Task LoadDashboardDataAsync()
        {
            ...
            // Load data sequentially to avoid token refresh race conditions
            // Load most important data first (assets)
            await LoadAssetsData();

            // Small delay to ensure any token refresh completes
            await Task.Delay(50);

            // Load other data sequentially
            await LoadRecentHistories();
            await LoadReferenceDataCounts();
        ...
        private async Task LoadAssetsData()
        {
            var assetsResponse = await _httpClient.GetAsync("api/assets?fields=basic");
            if (assetsResponse.IsSuccessStatusCode)
            {
                var assets = await assetsResponse.Content.ReadFromJsonAsync<List<AssetReadDTO>>() ?? new List<AssetReadDTO>();
            ...
            else if (assetsResponse.StatusCode == HttpStatusCode.Unauthorized)
            {
                _logger.LogWarning("Unauthorized access to assets API");
                SetDefaultData();
            }
        }
        ...
        private async Task LoadRecentHistories()
        {
            ...
            {
                _logger.LogWarning(ex, "Error loading recent histories");
                RecentActivities = 0;
        ...
        private async Task LoadReferenceDataCounts()
        {
            try
            {
                // Load sequentially to avoid token refresh conflicts
                TotalCategories = await GetCountFromResponse(await _httpClient.GetAsync("api/categories"));
                TotalLocations = await GetCountFromResponse(await _httpClient.GetAsync("api/locations"));
                TotalDepartments = await GetCountFromResponse(await _httpClient.GetAsync("api/departments"));

                // Users endpoint might not exist, so we set default
                TotalUsers = 0;

                // Optional: If you have a users count endpoint, uncomment:
                // TotalUsers = await GetCountFromResponse(await _httpClient.GetAsync("api/users/count"));
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Error loading reference data counts");
                TotalCategories = 0;
                TotalLocations = 0;
                TotalDepartments = 0;
                TotalUsers = 0;
        ...
        private async Task LoadAnalyticsData()
        {
            ...
            {
                _logger.LogWarning(ex, "Error loading analytics data");
            }
        }
        ...
        private async Task<int> GetCountFromResponse(HttpResponseMessage response)
        {
            ...
            {
                _logger.LogWarning(ex, "Error getting count from response");
                return 0;
            }
        }
        ...
        private void SetDefaultData()
        {
            ...
            MonthlyValueData = GenerateMonthlyValueData(new List<AssetReadDTO>());
            RecentAssetHistories = new List<AssetHistoryReadDTO>();
        }
    ...
    public class DashboardCache
    {
        ...
        public void Clear()
        {
            ...
            ConditionChartData.Clear();
            MonthlyValueData.Clear();
            RecentAssetHistories.Clear();
        }
    }
}