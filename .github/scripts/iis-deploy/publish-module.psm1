...
function Execute-Command {
    ...
    process{
        ...
        # Fail the deploy step when msdeploy/dotnet exits non-zero (otherwise CI stays green).
        if ($process.ExitCode -ne 0) {
            throw ("Command failed with exit code {0}: `"{1}`" {2}" -f $process.ExitCode, $exePath, $arguments)
        }
    }
}
